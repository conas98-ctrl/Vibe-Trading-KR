...
- **2026-06-04** 🇰🇷 **한국시장 자격증명 smoke 검증 Runbook 추가**: 한국 브로커/데이터 포팅에서 contract-only 구현과 실제 credentialed smoke를 구분하기 위한 [한국시장 자격증명 smoke 검증 Runbook](wiki/docs/ko/credential-smoke-runbook.md)을 추가했습니다. 브로커 connector smoke와 KRX/코스콤 데이터 smoke는 no-call baseline과 credentialed run을 분리해 raw evidence와 audit report를 함께 남기며, `broker_calls_proven` 또는 `data_calls_proven`이 `true`인 audit report가 있을 때만 실연동 완료로 표시합니다. 실계좌 주문 smoke는 명시 승인, mandate snapshot, kill switch 확인, 최소 주문/즉시 취소 evidence 뒤에서만 닫습니다.
- **2026-06-04** 🇰🇷 **KIS 미체결/정정취소 가능 주문 조회 계약 추가**: 한국투자증권 공식 샘플의 `inquire-psbl-rvsecncl`(`/uapi/domestic-stock/v1/trading/inquire-psbl-rvsecncl`, `TTTC0084R`)을 `get_open_orders`에 연결했습니다. 계좌번호/상품코드 기반 조회 파라미터(`INQR_DVSN_1=1`, `INQR_DVSN_2=0`, 연속조회 key 공란)를 mockable 테스트로 고정하고, 응답의 주문번호, 종목, 매수/매도, 주문수량, 체결수량, 정정취소 가능수량, 주문가를 Vibe-Trading open-order 딕셔너리로 정규화합니다. 실제 계정 연속조회와 주문/취소 smoke는 여전히 KIS 자격증명 게이트 뒤에 둡니다.
- **2026-06-04** 🇰🇷 **KIS 국내주식 WebSocket 계약 보강**: 한국투자증권 공식 `open-trading-api` 샘플에서 WebSocket 운영/모의 endpoint(`ws://ops.koreainvestment.com:21000` / `ws://ops.koreainvestment.com:31000`), `/oauth2/Approval` approval key 발급, 국내주식 실시간호가 KRX/NXT/통합, 실시간체결가 KRX/NXT/통합, 체결통보 실전/모의 TR ID(`H0STCNI0` / `H0STCNI9`)와 주요 예상체결·지수·장운영·회원사·시간외·프로그램매매 채널을 mockable 테스트로 고정했습니다. 구독 메시지는 공식 `data_fetch` 형태(`approval_key`, `tr_type`, `custtype`, `body.input.tr_id/tr_key`)를 따르며, 수신 프레임 파서는 KIS의 `|`/`^` data frame과 JSON system frame을 정규화합니다. 실제 WebSocket handshake와 체결통보 복호화 smoke는 KIS 계정/approval key가 준비된 뒤 닫습니다.
- **2026-06-04** 🇰🇷 **DB증권 WebSocket 계약 보강**: DB증권 공식 Open API JSON에서 국내주식 실시간 WebSocket 표면을 추가로 고정했습니다. 운영/모의 endpoint는 각각 `wss://openapi.dbsec.co.kr:7070` / `wss://openapi.dbsec.co.kr:17070`이며, `S00` 주식체결가, `S01` 주식호가, `IS0` 주식주문접수, `IS1` 주식주문체결 구독 메시지와 `DisconnectSession` 세션 초기화 REST 계약을 mockable 테스트로 고정했습니다. 실제 WebSocket 연결 smoke는 DB 계정/토큰이 준비된 뒤 닫습니다.
- **2026-06-04** 🇰🇷 **한국시장 백테스트 라우팅 보강**: 백테스트 시장 감지에 `kr_equity`를 추가해 `005930.KS`, `035720.KQ`, `KRX:005930`, `KR.005930` 형식을 `source="auto"`에서 한국 주식으로 라우팅합니다. yfinance loader는 한국 종목을 `.KS`/`.KQ` Yahoo 호환 심볼로 변환하고, 한국 주식 benchmark는 KOSPI `^KS11`로 해석합니다. Global equity engine에는 KRX 기본 규칙([거래 단위 1주, 일일 가격제한 ±30%, T+2 결제](https://global.krx.co.kr/contents/GLB/01/0109/0109000000/guide_to_trading_in_the_korean_stock_market.pdf)) 중 1주 단위, long-only 기본값, 가격제한 체결 차단을 반영했습니다. 수수료/거래세는 계좌·상품·시점별로 달라 `kr_commission`, `kr_transaction_tax`, `slippage_kr` 설정으로 명시 입력하게 두며, 공식 KRX/코스콤 데이터 loader와 T+2 현금 결제 잠금은 다음 검증 게이트로 남깁니다.
- **2026-06-04** 🇰🇷 **한국시장 포크 1차 부트스트랩**: 한국 브로커 포팅의 기반 레이어를 추가했습니다. KIS Open API, LS OpenAPI, DB증권 Open API, 키움 REST OpenAPI는 `broker_sdk` profile로 등록되고, 키움 OpenAPI+와 대신 CYBOS/CREON Plus는 Windows COM/OCX를 직접 호출하지 않는 `local_bridge` 읽기 전용 profile로 들어옵니다. mandate universe에는 `kr_equity`, `kr_etf`, `kr_derivative`, `kr_bond`, `kr_elw`가 추가되며, `005930` / `005930.KS` / `KRX:005930` 형식은 한국 주식 bucket으로 분류됩니다. KIS 국내주식 현재가, 기간별시세, 잔고조회, 현금주문, 정정취소 REST 계약은 공식 샘플 기반 테스트로 고정했습니다. LS는 토큰 발급, `t1101` 현재가, `t0424` 잔고조회, `CSPAT00601/CSPAT00701/CSPAT00801` 주문/정정/취소 REST 계약을 고정했고, DB증권은 `/oauth2/token`, `PRICE`, `CSPAQ03420`, `CSPAQ04800`, `CSPAT00600/CSPAT00700/CSPAT00800`, NXT `CSPAT00610/CSPAT00710/CSPAT00810` 국내주식 시세/잔고/체결·미체결/주문·정정·취소 계약을 공식 포털 JSON 기반 테스트로 고정했습니다. 키움 REST는 `ka10001` 주식기본정보, `ka10081` 일봉차트, `kt00018` 계좌평가잔고, `ka10075` 미체결, `kt10000/kt10001/kt10002/kt10003` 주문/정정/취소 계약을 공식 가이드 기반 테스트로 고정했습니다. 키움 OpenAPI+ / 대신 CYBOS local bridge는 `/health`, account, positions, orders, quote, history read-only HTTP 계약으로 고정했습니다. 추가 공식 조사표에는 토스증권 Open API 사전신청, 유진 챔피언 Open API, 유안타 티레이더 Open API, KB 핀테크스토어, 미래에셋 로보링크, KRX/코스콤 데이터 API 후보와 각 차단 게이트를 분리했습니다. 실제 주문/시세/잔고 실연동 완료는 브로커별 자격증명과 Windows 브리지 실행면이 준비된 뒤 별도 smoke test로 닫습니다.
...
# KIS WebSocket smoke 채널/TR 카탈로그를 broker call 없이 확인
vibe-trading connector kis-websocket-channels

# LS WebSocket stock channel/TR catalog를 broker call 없이 확인
vibe-trading connector ls-websocket-channels
```
...
Vibe-Trading은 모든 MCP-compatible client를 위해 41개 MCP tools를 제공합니다. stdio subprocess로 실행되므로 server setup이 필요 없습니다. 핵심 research tools는 HK/US/crypto에서 API key 없이 작동하고, trading connector tools는 선택된 connector profile을 사용하며, `run_swarm`만 LLM key가 필요합니다.
...
한국시장 확장에서는 KIS WebSocket 채널 카탈로그도 MCP에서 읽기 전용으로 확인할 수 있습니다. `trading_kis_websocket_channels`는 로컬 메타데이터만 반환하며 `network=not_attempted` 상태로 broker approval key 요청, WebSocket 연결, evidence 파일 쓰기를 하지 않습니다.
...
이 카탈로그로 credentialed smoke 전에 `ccnl_krx` / `H0STCNT0` 같은 체결 채널과 `ccnl_notice` / `H0STCNI0` / `H0STCNI9` 같은 체결통보 채널을 먼저 고를 수 있습니다.

한국시장 확장에서는 LS WebSocket 채널 카탈로그도 MCP에서 읽기 전용으로 확인할 수 있습니다. `trading_ls_websocket_channels`는 로컬 메타데이터만 반환하며 `network=not_attempted` 상태로 LS access token 요청, WebSocket 연결, evidence 파일 쓰기를 하지 않습니다.
...
이 카탈로그로 credentialed smoke 전에 `kospi_trade` / `S3_`, `kosdaq_orderbook` / `HA_`, `stock_order_execution` / `SC1` 같은 shortcut channel과 TR code를 먼저 고를 수 있습니다.
...
LS 국내주식 WebSocket smoke를 실행하거나 credentialed evidence를 남길 때는 [LS WebSocket Smoke Runbook](https://vibetrading.wiki/docs/latest/tools/ls-websocket-smoke-runbook)을 먼저 확인하세요. 기본값은 broker call을 하지 않으며, 실제 LS Open API 호출은 `--allow-broker-calls`와 실전 profile의 `--allow-live`로 명시적으로 열립니다.
...
**노출되는 MCP tools(41):** `list_skills`, `load_skill`, `start_research_goal`, `get_research_goal`, `add_goal_evidence`, `update_research_goal_status`, `backtest`, `factor_analysis`, `analyze_options`, `pattern_recognition`, `read_url`, `read_document`, `web_search`, `write_file`, `read_file`, `trading_kiwoom_websocket_channels`, `trading_kiwoom_websocket_smoke`, `trading_kis_websocket_channels`, `trading_kis_websocket_smoke`, `trading_ls_websocket_channels`, `trading_connections`, `trading_select_connection`, `trading_check`, `trading_account`, `trading_positions`, `trading_orders`, `trading_quote`, `trading_history`, `list_swarm_presets`, `run_swarm`, `get_market_data`, `get_swarm_status`, `get_run_result`, `list_runs`, `reap_stale_runs`, `retry_run`, `analyze_trade_journal`, `extract_shadow_strategy`, `run_shadow_backtest`, `render_shadow_report`, `scan_shadow_signals`.
