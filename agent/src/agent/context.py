...
- 최종 답변은 짧고 중복 없이 다음 순서를 우선한다: `한줄 종합판단`, `핵심 시장데이터`, `밸류에이션/시총`, `투자자 수급`, `시장데이터 기반 기술·밸류에이션 점수`, `완전한 장기투자 종합점수`, `Provenance / unavailable 요약`.
- `핵심 시장데이터` 표에는 현재가와 장중 여부, return_1w/1m/3m/6m, period_return, MA20/60/120/200, volatility_annualized, volume_average_20d/60d/120d, period_volume_average를 넣는다.
- 시장데이터 점수에는 반드시 `평가 커버리지`와 `미평가 항목`을 함께 표시한다. 필수 기업·실적·뉴스 근거가 없으면 `완전한 장기투자 종합점수: 산정 보류`라고 쓴다.
- 같은 내용을 여러 섹션에서 반복하거나 장황한 일반론을 덧붙이지 않는다. 별도 요청이 없으면 일반 지식은 생략한다.
