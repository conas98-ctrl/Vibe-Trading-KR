...
@mcp.tool
def trading_ls_websocket_smoke(
    channel: str,
    tr_key: str,
    evidence_path: str,
    connection: str | None = None,
    ...
    """Run gated LS WebSocket smoke/evidence through the selected profile.
    ...
    Args:
        channel: LS WebSocket channel key such as kospi_trade.
        tr_key: Subscription key such as a Korean stock code.
        evidence_path: Local JSON path for redacted smoke evidence.
        connection: Optional LS broker SDK profile id. Defaults to the selected profile.
        host: Optional local host override.
        port: Optional local socket port override.
        client_id: Optional local client id override.
        account: Optional account code filter.
    ...
    channel_key = str(channel or "").strip().lower()
    if channel_key not in LS_WEBSOCKET_CHANNELS:
        return json.dumps(
            {
                "status": "error",
                "error_type": "validation",
                "error": f"unsupported LS WebSocket channel: {channel!r}",
                "supported_channels": sorted(LS_WEBSOCKET_CHANNELS),
            },
            ensure_ascii=False,
            indent=2,
        )
    ...
    if message_timeout is not None:
        params["message_timeout"] = message_timeout
    registry = _get_registry()
    return registry.execute("trading_ls_websocket_smoke", params)
